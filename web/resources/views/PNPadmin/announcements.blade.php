        ...
        <div class="content-wrapper">
            ...
            <div class="modal fade" id="add-announcement" tabindex="-1" role="dialog" aria-labelledby="exampleModalFormADD" aria-hidden="true" >
                <div class="modal-dialog" role="document">
                    <div id="add-announcement-loader" class="col-sm-12" style="display: none;">@include('PNPadmin.includes.loader')</div>
            ...
            <div class="modal fade" id="update-announcement" tabindex="-1" role="dialog" aria-labelledby="exampleModalFormADD" aria-hidden="true" >
                <div class="modal-dialog" role="document">
                    <div id="update-announcement-loader" class="col-sm-12" style="display: none;">@include('PNPadmin.includes.loader')</div>
            ...
            <div class="modal fade" id="remove-announcement" tabindex="-1" role="dialog" aria-labelledby="exampleModalFormADD" aria-hidden="true" >
                <div class="modal-dialog" role="document">
                    <div id="remove-announcement-loader" class="col-sm-12" style="display: none;">@include('PNPadmin.includes.loader')</div>
                        <form id="remove-announcement-form">
                        @csrf
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalFormTitle">Are you sure you want to remove this Announcement?</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
...
<script>
  $(document).ready(function() {
    var table = $('#responsive-data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ url('admin/post/announcements') }}",
        },
        columns: [
...
$('#add-announcement-form').on('submit', function (event) {
    ...
    $.ajax({
        ...
        url: "{{ url('admin/post/addannouncement') }}",
...
$('#update-announcement-form').on('submit', function (event) {
    ...
    var update_announcement_id = $('.modal-body #update-announcement-id').val();

    $.ajax({
        type: "POST",
        enctype: "multipart/form-data",
        url: "/admin/post/updateannouncement/"+update_announcement_id,
        data: new FormData(this),
        dataType: "JSON",
        contentType: false,
        cache: false,
        ...
        success: function(result) {
            ...
            $('#responsive-data-table').DataTable().ajax.reload();
            setTimeout(function() {
            $('.modal-body #update-announcement-success').fadeOut('slow');
            }, 3000);
        },
        error: function(error) {
            alert('ERROR! DEBUG NOW!')
        },
    });
});
...
$('#remove-announcement-form').on('submit', function (event) {
    ...
    var remove_announcement_id = $('.modal-body #remove-announcement-id').val();

    $.ajax({
        type: "POST",
        enctype: "multipart/form-data",
        url: "/admin/post/removeannouncement/"+remove_announcement_id,
        data: new FormData(this),
        dataType: "JSON",
        contentType: false,
        cache: false,
